...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
str(data)
m <- lm(y ~ imp + aft + imp:aft, data = data)
summary(m)

# can also be written as 
m <- lm(y ~ imp*aft, data = data)
summary(m)
...
p <- data.frame(p)
plot(p$fit, xaxt = 'n', ylim = c(-3,6), xlim = c(0.5,4.5),
...
# add means
points(p$fit, pch = 21, bg = c('white','red','white','red'), cex = 2.5)
abline(v = 2.5, lty = 2)
text(1.5,6,'Before')
text(3.5,6,'After')
...
jags.data <- list(y = data$y, n = nrow(data), 
                  imp = data$imp, aft = data$aft)
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# ~10k iterations takes about 20 minutes on my desktop for 10k iterations
# 12th Gen Intel(R) Core(TM) i9-12900K (3.20 GHz) 64 gb RAM
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
library(jagsUI)
Sys.time()
m <- jags(jags.data, inits, parameters, "model.jags", 
...
# add means
points(q[3,], pch = 21, bg = c('white','red','white','red'), cex = 2.5)
abline(v = 2.5, lty = 2)
text(1.5,6,'Before')
text(3.5,6,'After')
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
par(mfrow = c(1,1), mar = c(5.1,5.1,2.1,2.1))
plot(NA, xaxt = 'n', ylim = c(-3,6), xlim = c(0.5,2.5),
...
# add confidence intervals
arrows(c(0.9,1.1,1.9,2.1), q[1,], c(0.9,1.1,1.9,2.1), q[5,], length = 0, lwd = 4)
# add means
points(q[3,] ~ c(0.9,1.1,1.9,2.1), pch = 21, bg = c('white','red','white','red'), cex = 2.5)
