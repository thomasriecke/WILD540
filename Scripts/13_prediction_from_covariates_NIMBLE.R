...
str(dataList <- list(y = data$y, x = data$x, 
                     n = n)) 
...
system.time(
  mB <- nimbleMCMC(code = model, constants = dataList,
                   inits = inits, monitors = params, nburnin = nb, niter = ni,
                   thin = nt, nchains = nc, samplesAsCodaMCMC = TRUE, summary = TRUE) )

lapply(mB, head) # print first 5 values of each chain (not shown)


# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# output predictions
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
tmp <- do.call(rbind.data.frame, mB$samples)
...
for (j in 1:ncol(py)){
  py[,j] <- exp(tmp$'alpha[1]' + tmp$'alpha[2]' * px[j])
}
z$pB <- colMeans(py)
plot(z['pB'], 
     main = NULL,                           # title for plot (NULL to remove)
